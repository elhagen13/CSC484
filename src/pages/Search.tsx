...
function SearchPage() {
  ...
  return (
    <div style={{ width: '100%', padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px', marginTop: '20px' }}>
        <div style={{width: '100%', height: 40, color: 'white', fontSize: 30, fontFamily: 'Inter', fontWeight: '900', wordWrap: 'break-word'}}>
          ...
          {filteredEvents.map((event) => (
            <div key={event.id} style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', border: '1px solid #D1D1D1', padding: '20px', borderRadius: '10px' }}>
              ...
              <div style={{ textAlign: 'left', flex: 1 }}>
                ...
                <div style={{ fontSize: '12px', color: '#A0A0A0', marginBottom: '1px' }}>
                  {event.date} @ {event.time}
                </div>
                
                {/* Event Title */}
                <div style={{ fontWeight: 'bold', fontSize: '18px' }}>{event.eventName}</div>
